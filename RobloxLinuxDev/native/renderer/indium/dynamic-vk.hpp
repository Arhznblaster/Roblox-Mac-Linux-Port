...
namespace Indium {
	namespace DynamicVK {
		...
		VkDeviceSize allocatedSize(VkDevice device);
		...
		#define INDIUM_DYNAMICVK_FUNCTION_FOREACH(_macro) \
			...
			_macro(vkCmdPushConstants) \
			_macro(vkCmdPushDescriptorSetKHR) \
			_macro(vkCmdDispatch) \
			_macro(vkCmdDraw) \
			_macro(vkCmdDrawIndexed) \
			_macro(vkCmdEndRenderPass) \
			_macro(vkCmdEndRendering) \
			...
			_macro(vkCmdSetDepthClampEnableEXT) \
			...
			_macro(vkEnumerateInstanceExtensionProperties) \
			...
			_macro(vkGetPhysicalDeviceSurfaceSupportKHR) \
			_macro(vkGetPipelineCacheData) \
