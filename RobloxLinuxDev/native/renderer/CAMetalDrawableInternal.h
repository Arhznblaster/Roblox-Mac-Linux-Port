...
class CAMetalDrawableActual: public Indium::Drawable {
	// Preserve submit-thread lifetime callbacks; import GL semaphores only on
	// the owning render thread immediately before their GPU wait.
	friend void trackb_present_drawable(std::shared_ptr<Indium::Drawable>, uint64_t);
	friend void trackb_prepare_render(id, SEL);
private:
