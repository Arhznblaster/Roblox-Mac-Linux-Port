...
void DarlingServer::MessageQueue::pushOrSend(int socket, Message&& message) {
	std::unique_lock lock(_lock);
	// A reply used to wait for the main loop: worker -> eventfd -> main loop ->
	// sendmmsg, an extra thread handoff on every RPC. Messages with a post-send
	// callback (kqchan) keep the queued path so the callback still runs there.
	if (_messages.empty() && !message.sendNotificationCallback()) {
		struct msghdr header = message.rawHeader();
		auto address = reinterpret_cast<sockaddr_un*>(header.msg_name);
		...
		ssize_t ret;
		do {
			ret = sendmsg(socket, &header, MSG_DONTWAIT);
		} while (ret < 0 && errno == EINTR);

		// A dead peer drops the message, exactly like sendMany().
		if (ret >= 0 || errno == EPIPE || errno == ECONNREFUSED) {
			return;
		}
		if (errno != EAGAIN) {
			throw std::system_error(errno, std::generic_category(), "Failed to send message through socket");
		}
	}
