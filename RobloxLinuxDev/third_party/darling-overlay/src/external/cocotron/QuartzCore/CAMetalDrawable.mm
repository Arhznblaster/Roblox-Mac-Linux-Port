...
void CAMetalDrawableActual::present() {
	if (_queued) {
		if (auto callback = std::exchange(_wantsToPresentCallback, nullptr)) {
			callback();
		}
		if (auto callback = std::exchange(_didPresentCallback, nullptr)) {
			callback();
		}
		return;
	}
...
// A callback releases this drawable for reuse before it returns. Take it out
// first so an old completion cannot erase the next frame's callback.
...
void CAMetalDrawableActual::reset() {
	if (_glSemaphore != 0) {
		...
		if (CGLSetCurrentContext(_glContext) != kCGLNoError) abort();
		reportGLErrors();
