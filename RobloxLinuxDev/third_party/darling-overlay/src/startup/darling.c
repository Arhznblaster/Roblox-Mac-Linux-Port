...
int main(int argc, char ** argv)
{
	...
	// The guest cannot report its exit if darlingserver dies first. Watch the
	// server independently so this CLI (our outer PID-namespace init) exits too.
	server_pidfd = syscall(SYS_pidfd_open, pidInit, 0);
	if (server_pidfd == -1)
	{
		perror("Cannot watch darlingserver");
		return 1;
	}
...
{
	// Child exits are handled by the server pidfd, not forwarded to the guest.
	if (signo == SIGCHLD)
		return;
...
static void shellLoop(int sockfd, int master)
{
	...
	pfds[2].revents = 0;
	pfds[1].fd = (master != -1) ? STDIN_FILENO : -1;
	pfds[1].events = POLLIN;
	pfds[1].revents = 0;
	pfds[0].fd = sockfd;
	pfds[0].events = POLLIN;
	pfds[0].revents = 0;
	pfds[3].fd = server_pidfd;
	...
	while (1)
	{
		...
		if (pfds[3].revents)
		{
			fprintf(stderr, "darlingserver exited before the guest command completed\n");
			exit(1);
		}
