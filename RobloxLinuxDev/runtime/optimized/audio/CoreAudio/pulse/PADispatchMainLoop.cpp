...
PADispatchMainLoop::PADispatchMainLoop()
{
	...
	dispatch_queue_set_specific(m_queue,this,this,nullptr);
...
pa_io_event* PADispatchMainLoop::io_new(pa_mainloop_api *a, int fd, pa_io_event_flags_t events, pa_io_event_cb_t cb, void *userdata)
{
	...
	// Pulse may close/reuse fd immediately after io_free. Dispatch owns this
	// duplicate until both source cancellation handlers have retired.
	dual->watchFd = fcntl(fd, F_DUPFD_CLOEXEC, 0);
	if (dual->watchFd < 0) { delete dual; return nullptr; }
	...
	dual->sourceRead = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, dual->watchFd, 0, q);
	if (!dual->sourceRead) { close(dual->watchFd); delete dual; return nullptr; }
	dual->sourceWrite = dispatch_source_create(DISPATCH_SOURCE_TYPE_WRITE, dual->watchFd, 0, q);
	if (!dual->sourceWrite) {
		dispatch_source_set_cancel_handler(dual->sourceRead, ^{ close(dual->watchFd); delete dual; });
	...
	// Both sources hold callbacks referring to this record. Their cancellation
	// handlers share the serial queue, so release it only after both finish.
	auto cancelled = ^{
		if (--dual->pendingCancels) return;
		close(dual->watchFd);
		if (dual->destroy)
		{
	...
	dispatch_source_set_cancel_handler(dual->sourceRead, cancelled);
	dispatch_source_set_cancel_handler(dual->sourceWrite, cancelled);
...
void PADispatchMainLoop::io_free(pa_io_event *e)
{
	...
	// dual is freed after both cancel handlers have run.
}
...
{
	if(!tv) {dispatch_source_set_timer(reinterpret_cast<pa_timer*>(e)->source,DISPATCH_TIME_FOREVER,0,0);return;}
	const linux_timeval* real_tv = reinterpret_cast<const linux_timeval*>(tv);
...
void PADispatchMainLoop::perform(void (^cb)(), bool sync) {
 if(dispatch_get_specific(this))cb();
 else if(sync)dispatch_sync(m_queue,cb);
 else dispatch_async(m_queue,cb);
}
