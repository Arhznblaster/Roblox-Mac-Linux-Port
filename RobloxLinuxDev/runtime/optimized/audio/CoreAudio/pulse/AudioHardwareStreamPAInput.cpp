...
void AudioHardwareStreamPAInput::paStreamReadCB(pa_stream* s, size_t length, void* self)
{
	...
	while (true)
	{
		...
		// AUBase caches rendered audio by sample time; each captured chunk must advance it.
		AudioTimeStamp time = This->timeStamp();
		OSStatus status = This->m_callback(This->m_hw->id(), &time, abl, &time, nullptr, nullptr, This->m_clientData);
		This->m_sampleTime += nbytes / This->m_format.mBytesPerFrame;
...
void AudioHardwareStreamPAInput::start()
{
	...
	int rv = pa_stream_connect_record(m_stream, static_cast<AudioHardwareImplPA*>(m_hw)->deviceName(), &battr,
