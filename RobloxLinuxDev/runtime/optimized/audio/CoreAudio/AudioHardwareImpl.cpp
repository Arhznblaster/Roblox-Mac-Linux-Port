...
: m_myId(myId)
{
	...
	m_asbd.mFormatFlags = kLinearPCMFormatFlagIsFloat | kAudioFormatFlagIsPacked | kAudioFormatFlagsNativeEndian;
...
{
 std::lock_guard<std::recursive_mutex> lock(m_formatMutex);
	...
	*outIsSettable = false;
	return hasProperty(inAddress)?noErr:kAudioHardwareUnknownPropertyError;
}
...
{
	std::vector<char> utf8(CFStringGetMaximumSizeForEncoding(CFStringGetLength(str),kCFStringEncodingUTF8)+1);
	CFStringGetCString(str,utf8.data(),utf8.size(),kCFStringEncodingUTF8);
