...
public:
	...
	uint32_t bufferSize() const { std::lock_guard<std::recursive_mutex> lock(m_formatMutex); return m_bufferSize; }

	AudioObjectID id() const { return m_myId; }
	AudioStreamBasicDescription asbd() const { std::lock_guard<std::recursive_mutex> lock(m_formatMutex); return m_asbd; }
private:
...
protected:
	...
	mutable std::recursive_mutex m_formatMutex;
